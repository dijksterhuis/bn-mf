...
private _fnc_burnShelter = {
	params ["_task"];

	diag_log format ["BURNING SHELTER!"];

	private _nearPlayers = (getPos _task) nearObjects ["Man", 10];
	{
		if (isPlayer _x) then {
			["CampBurnt", ["A camp shelter has been set on fire!"]] remoteExec ["para_c_fnc_show_notification", _x];
		};
...
if(typeOf _task == "Land_vn_o_shelter_06" && call _fnc_hasLighter) exitWith {
	diag_log format ["REMOTING BURNING SHELTER!"];
	[_task] remoteExec ["_fnc_burnShelter", 0];

	sleep 20;
	diag_log format ["SPAWNING BURNING SHELTER!"];
	[_task] spawn _fnc_burnShelter;
};
